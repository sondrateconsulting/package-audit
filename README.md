...
- **Repo read access** via a stock `gh auth login` — the tool reads branch listings and commit metadata through `gh`, and repository content through per-unit read-only `git` clones authenticated by `gh`'s credential helper (symlink targets alone fall back to the REST contents API); it exercises no write permission of any kind.
...
| `repositories` | 8 | **Global** in-flight cap on one-shot `gh`/`git`/`tar` subprocesses — and, separately, the size of the unit-lived content-child pool (see below). |

`repositories` is the network-politeness governor, **not** a repo-loop parallelism degree despite its name: no matter how `organizations × branches` compose, at most `repositories` **one-shot** subprocess leases are ever held at once. Content reads add a second, independent pool of the same size: a scanning unit that reads canonical blobs keeps one long-lived `git cat-file --batch` child for its read phase (spawned lazily at the first such read, so a unit with nothing to read never starts one), and those children deliberately hold no one-shot slot (a child holding one while its unit awaited a REST fallback would deadlock). Both pools are bounded and independent of the fan-out widths, so the worst-case managed-subprocess count is `repositories × 2` — eight one-shot leases alongside eight content children at the default. (The bound is on leases, not a hard OS ceiling: on abnormal teardown paths a kill-escalated child whose exit never settled can transiently outlive its released lease.) So the two fan-out knobs govern how much work is *dispatched*, while `repositories` bounds the actual `gh`/`git`/`tar` subprocess concurrency (an immutable, SHA-pinned cache hit is served with no subprocess, so it consumes no slot; a conditional revalidation still spends one on its `304` round-trip). npm-registry fetches use their own request timeout and run outside this cap. Higher values speed up large estates; `organizations: 1, branches: 1` reproduces a fully sequential run — one owner, one branch-unit at a time, and a fatal stops dispatching further units at once. `--plan` traversal is always sequential regardless of these widths.
...
**stdout is pure JSONL during audit and plan runs** — one structured event per line, safe to pipe. Vocabulary: `config`, `concurrency` (the configured fan-out widths — `organizations`, `branches`, `repositories`; emitted for both audit and `--plan`, though `--plan` traversal is sequential; see [Concurrency](#concurrency)), `preflight`, `owners`, `run`, `rescan-branch`, `cli-terms`, `plan`, `plan-excluded` (a `--plan`-only, per-owner list of the `owner/repo` names dropped by the `excludeRepositories` denylist — emitted only when that owner had exclusions, so a developer can confirm each pattern caught the repositories it meant; the aggregate `repositoriesExcluded` count rides on `plan-summary`), `plan-summary`, `unit` (actions `scanned`, `skip-current`, `skip-cutoff`, `skip-policy`, `past-cap`, `error`, `requeue-throttle`), `content-transport` (one line per unit that enters the scan path — skipped units return before it — carrying the ADR-0001 separated transport counters: `localCanonicalReads`, `symlinkFallbackReads`, `fallbackBudgetSpend` against `fallbackBudget`, `cloneAttempts`/`cloneRetries`, `childRespawns`, and an `outcome` of `scanned` or `failed`. A unit that fails still reports the transport it spent — a clone that exhausts its attempts made those gated launch attempts (counted immediately before each spawn, so a launch that dies before transport still counts), and its `cloneAttempts`/`cloneRetries` are part of the pacing account; counters the failure preceded read zero), `reconciliation` (actions `prune-stale` — a repo-scoped line emitted when a resume prunes `run_unit_head` rows for branches deleted since a prior invocation, so a re-discovered repo's rows reflect its live branch set; the prune runs per repo, so a repo that is itself no longer discovered or kept this run — deleted, renamed, newly archived/fork-filtered, or displaced past `maxReposPerOrg` — is never revisited and keeps its prior rows — and `prune-excluded-owner` — a run-scoped line (with `prunedHeads`/`prunedErrors` counts) emitted when a resume drops an owner's `run_unit_head` AND `errors` rows because `excludeOrganizations` now excludes it case-insensitively but did not before a prior invocation, so a resumed run never reports an excluded owner in its findings, dispositions, or errors), `discovery`, `introspection`, `warning` (a `reason` names the trigger — `fresh-dropped-completed-runs` when `--fresh` drops completed runs, `temp-sweep-failed` when the startup temp-dir sweep can't list or reclaim a stale clone directory, or RETAINS one whose owner marker exists but could not be read — it may be a live sibling audit's, so the sweep fails safe and surfaces the ambiguity instead of deleting, `clone-cleanup-failed` when a per-unit clone temp dir can't be reclaimed after the scan, `content-store-unclean` when a unit that is ALREADY failing for another reason also closed its content child uncleanly — the close diagnosis rides beside that primary error rather than replacing it; when the unclean close is itself the reason the unit failed, it IS the unit's error and no separate warning is emitted), `policy-warning` (advisory branch-policy notices — kinds `unmatched-pattern` for a configured `branches`/`excludeBranches` pattern that matched no branch discovered this run, `default-only-deny` for an `excludeBranches` pattern whose only matches were default branches (which are always scanned, so it excluded nothing) — both rules doing nothing — and `empty-allowlist`, the opposite: `branches: []` drops every non-default branch), `owner-discovery-throttled`, `done`. When a request's retry/pause budget is exhausted — GitHub rate limiting, or repeated HTTP 5xx — that work is **deferred, not failed**: a `unit` or `discovery` line carries `action` `requeue-throttle`, and an `owner-discovery-throttled` event carries `action` `retry-next-run` — the affected units are retried by a later run that re-enumerates them (a resumed run skips already-scanned units). The end-of-run report makes that remainder visible: `summary.branchesDeferred` (also on the `done` event) counts the config's still-pending branch units when the report was built, and the stderr summary prints a `Branches deferred` line that flags the scanned counts as PARTIAL when it is nonzero. A throttled owner/repo *discovery* enqueues nothing, so branches it never enumerated stay outside that count; conversely a pending unit no future run re-enumerates — its branch since deleted, its repo out of the kept set (deleted, renamed, newly archived, fork-filtered, or displaced past `maxReposPerOrg`), or its owner gone — lingers inside it until a run re-encounters it (a re-encountered branch is scanned, skipped, settled — or throttled again and left pending). That discovery blind spot has its own evidence: `summary.discoveryScopesDeferred` (also on the `done` event, and persisted as `runs.discovery_scopes_deferred`) counts the DISTINCT discovery scopes — one per owner repo-listing, one per repository branch-listing — that exhausted their bounded retry/pause budget during the run (a call running out of retries under rate limiting or repeated HTTP 5xx, or the run spending its cumulative pause budget), and a `Discovery scopes deferred` line flags the counts PARTIAL when it is nonzero. It counts *scopes*, not branches (one throttled owner listing can hide any number of repositories), so never add it to a branch count. The two also differ in currency: `branchesDeferred` is the whole config's backlog re-read whenever the report is generated, while `discoveryScopesDeferred` is immutable evidence sealed when that run completed. **`null` means UNKNOWN, never zero** — the run is still running, it failed, it predates schema v5, or it was *resumed* and the completing invocation saw no throttles (the accumulator lives in memory for one invocation, so it cannot vouch for an earlier one). Every surface renders that as "unknown"/"not recorded" rather than a reassuring 0. On a fresh run a positive value is exact; on a resumed run it is a **lower bound** — at least that many scopes exhausted their budget.
...
**Mid-run `"action":"error"`, `"event":"discovery"`, and `"event":"introspection"` lines are fail-soft**: the failure (a branch scan, a repo/branch listing for one owner, or a registry packument/tarball/bin-discovery step for one package version) is recorded in the report's `errors` array and the run continues — one unreachable repo or registry hiccup never kills an org-wide audit. The one deliberate exception is a retry/pause-budget exhaustion (rate limiting, repeated HTTP 5xx, or the run's cumulative pause budget): that is transient, not a failure, so it records no `errors` entry — a throttled unit is left pending (and counted by `summary.branchesDeferred`), while a throttled owner/repo *listing* enqueues nothing at all and is counted by `summary.discoveryScopesDeferred`. Org-scoped `discovery` lines (a whole owner's repo listing failed) carry no `repo` field; branch-scoped ones do. `introspection` lines are package-scoped (`packageName`, plus `version` when the failure is version-specific). The human-readable end-of-run summary prints to **stderr**.
...
2. **Two sanctioned spawn sites, grep-enforced**: a repo-wide test fails if any file other than the audit wrapper ([scripts/github.ts](scripts/github.ts)) — or the ADR-0001 benchmark harness's single, separately-gated git launcher ([scripts/benchSpawn.ts](scripts/benchSpawn.ts), allowlisted by exact repo-relative path and imported by no audit entrypoint) — reaches a spawn surface in its source text, with two SCANNER test files exempted wholesale by exact path (`github.test.ts`, `tuiPurity.test.ts`), which must spell the very tokens they assert on and one of which itself spawns — a dotted, optional-chained, or whitespaced `Bun.spawn`/`spawnSync`/`$`; any `"bun"`-module import (quote or backtick); `Bun` aliased, parenthesized, bracket-indexed, or reached via `globalThis.Bun`; `child_process` in any form; or a dynamic import whose specifier is a bare variable/expression or is built with `+`/`${}` ([scripts/github.test.ts](scripts/github.test.ts), "spawn-site allowlist"). It is a best-effort textual tripwire, **not** a semantic proof: it catches the common direct routes but not deliberately evasive ones — a token split across comments, a module name assembled by other means (`.concat`, char codes), or the `Bun` global routed through several intermediate bindings — which are the domain of code review, not this grep. The load-bearing read-only guarantee is guarantee 1 (the argv allowlist, which validates every invocation that goes *through* the wrapper); this tripwire's job is to keep every spawn in the files it scans inside the two sanctioned sites by failing the common bypasses in CI — the two exact-path scanner-test exemptions above sit outside that scan, and one of them spawns.
...
Those are single-repo hygiene tools: they find unused dependencies and dead exports so you can clean up one project's `package.json`, and they're good at it. This is org-wide and evidence-producing — it attempts every repo and branch in scope (your configured cutoff and caps define that scope; archived repos and forks are excluded by default) and attributes usage sites to specific exports where the import syntax names one, each with a commit-pinned permalink you can hand to someone else.
