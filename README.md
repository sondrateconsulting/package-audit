...
6. **Sanitized child environment (allowlist)** ([scripts/github.ts](scripts/github.ts), `buildGhEnv`/`buildGitEnv`/`buildTarEnv`): `gh`/`git`/`tar` run with a strict env allowlist, not your inherited environment. Only three classes are copied — **auth** (`GH_TOKEN`/`GITHUB_TOKEN`/enterprise variants, `GH_CONFIG_DIR`/`XDG_CONFIG_HOME`), **transport** (the `HTTP(S)_PROXY`/`NO_PROXY`/`ALL_PROXY` family in both upper- and lower-case; the OpenSSL `SSL_CERT_FILE`/`SSL_CERT_DIR` pointers; for git also the libcurl `GIT_SSL_CAINFO`/`GIT_SSL_CAPATH`/`CURL_CA_BUNDLE` CA-bundle pointers; and `DBUS_SESSION_BUS_ADDRESS`/`XDG_RUNTIME_DIR` for the OS keyring the credential helper reads), and **benign terminal** vars. Everything else is dropped — including `GIT_ASKPASS`/`GIT_SSH_COMMAND`/`TAR_OPTIONS` and any other injection vector — and git additionally pins **all** of its config (system → `/dev/null`, global → a generated read-only credential-helper config). The transport vars are copied so the tool works behind a corporate proxy or a TLS-inspecting root; they govern only *how it connects*, and cannot grant write access, inject git config, or redirect where it writes (guarantees 1, 4 still hold). Registry packument/tarball fetches use Bun's built-in `fetch`, which honors `HTTPS_PROXY` and the system trust store; `NODE_EXTRA_CA_CERTS` support is Bun-version-dependent (the ≥ 1.1 floor doesn't guarantee it), so behind a TLS-inspecting proxy prefer `SSL_CERT_FILE` or a newer Bun.
