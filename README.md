...
- A grep-enforced test ([scripts/tuiPurity.test.ts](scripts/tuiPurity.test.ts)) pins the display layer away from the machine stdout stream, the filesystem write APIs (one audited exception: the contained divert-log writer), the spawn surface, and the DB/GitHub modules; the repo-wide single-chokepoint scan walks the dashboard sources too. Those scans cover **this repo's code**; one known dependency edge is worth naming: ink's bundled `terminal-size` helper can shell out (`tput`) as a *fallback* when its render stream reports no dimensions. This tool's own code never invokes that helper (the dashboard reads dimensions directly from the stream and renders an empty frame when they're absent), and the dashboard only ever mounts on a real TTY, which reports its size — but it is ink's code, inside the mounted process, and honesty beats silence here.
