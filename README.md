...
A run always writes `output/run-<run_id>.json`; a **complete** run (full estate coverage) also writes `output/latest.json`, while a **partial** run (rate-limiting or a discovery gap left units un-scanned — the process exits 75) writes only its own `run-<id>.json` and leaves `latest.json` untouched, so "latest" is never a partial view (see [Reading a run](#reading-a-run) for the run-outcome and exit-code contract). A long first run is normal for large orgs — interrupt it any time; the next invocation resumes where it left off and re-runs skip unchanged branches. `bun run report` exists to (re-)emit reports from the database: the default form refreshes `latest.json`, and `--run-id <id>` re-emits a historical run. The audit already emits the report itself. Both entrypoints take `--help`.
...
**stdout is pure JSONL during audit and plan runs** — one structured event per line, safe to pipe. Every event carries a `ts` (ISO-8601 UTC) as its first key. Vocabulary: `config`, `concurrency` (the configured fan-out widths — `organizations`, `branches`, `repositories`; emitted for both audit and `--plan`, though `--plan` traversal is sequential; see [Concurrency](#concurrency)), `preflight`, `owners`, `run`, `rescan-branch`, `cli-terms`, `phase`, `plan`, `plan-excluded` (a `--plan`-only, per-owner list of the `owner/repo` names dropped by the `excludeRepositories` denylist — emitted only when that owner had exclusions, so a developer can confirm each pattern caught the repositories it meant; the aggregate `repositoriesExcluded` count rides on `plan-summary`), `plan-summary`, `unit` (actions `start`, `scanned`, `skip-current`, `skip-cutoff`, `skip-policy`, `past-cap`, `error`, `requeue-throttle`), `repo-done`, `reconciliation` (actions `prune-stale` — a repo-scoped line emitted when a resume prunes `run_unit_head` rows for branches deleted since a prior invocation, so a re-discovered repo's rows reflect its live branch set; the prune runs per repo, so a repo that is itself no longer discovered or kept this run — deleted, renamed, newly archived/fork-filtered, or displaced past `maxReposPerOrg` — is never revisited and keeps its prior rows — and `prune-excluded-owner` — a run-scoped line (with `prunedHeads`/`prunedErrors` counts) emitted when a resume drops an owner's `run_unit_head` AND `errors` rows because `excludeOrganizations` now excludes it case-insensitively but did not before a prior invocation, so a resumed run never reports an excluded owner in its findings, dispositions, or errors), `discovery`, `introspection`, `warning` (a `reason` names the trigger — `fresh-dropped-completed-runs` when `--fresh` drops completed runs, `temp-sweep-failed` when the startup temp-dir sweep can't list or reclaim a stale clone directory, `clone-cleanup-failed` when a per-unit clone-fallback temp dir can't be reclaimed after the scan), `policy-warning` (advisory branch-policy notices — kinds `unmatched-pattern` for a configured `branches`/`excludeBranches` pattern that matched no branch discovered this run, `default-only-deny` for an `excludeBranches` pattern whose only matches were default branches (which are always scanned, so it excluded nothing) — both rules doing nothing — and `empty-allowlist`, the opposite: `branches: []` drops every non-default branch), `owner-discovery-throttled`, `run-deferred`, `throttle`, `retry`, `spawn-timeout`, `heartbeat`, `done`. When GitHub rate-limiting persists past the retry budget, throttled work is **deferred, not failed**: a `unit` or `discovery` line carries `action` `requeue-throttle`, and an `owner-discovery-throttled` event carries `action` `retry-next-run` — the affected units are finished on the next run (a resumed run skips already-scanned units).

**Run outcome and exit code.** Every run ends by declaring its coverage. The terminal `done` event carries a `runOutcome` — `complete` (the whole estate was covered), or a `partial-*` variant (`partial-deferred` when rate-limiting or a discovery gap left units un-scanned) — and the process exits **0** for a complete run or **75** (`EX_TEMPFAIL`) for a partial one, so a CI wrapper can tell "finished" from "finished, re-run to complete". A pre-run deferral (rate-limiting blocks owner discovery before any scan starts) emits a terminal `run-deferred` event and also exits 75, with no run recorded. A partial run still writes its own `run-<id>.json` report but does **not** overwrite `latest.json` — only a complete run is served as "latest". An unrecoverable error exits **1**.

Transient network hiccups are visible but flood-controlled. A `retry` line (`reason` `no-response` or `http-5xx`) marks a bounded retry of one call; a `throttle` line marks a rate-limit pause (`kind` `primary`/`secondary`, `bucket`, `untilMs`); a `spawn-timeout` line marks a killed `gh`/`git`/`tar` child that outran its deadline. On a flaky link these can be frequent, so `retry`/`throttle` are globally rate-limited to a trickle while running totals accumulate — the `heartbeat` and `done` events carry `retryTotal` (every retry attempted) and `suppressed` (lines the rate-limiter or a slow stdout consumer held back — telemetry first, but if the stdout buffer saturates entirely with non-droppable events it can include a lifecycle/unit line too), so the full churn is always accounted for even though not every line is printed. stdout is live telemetry; the database and report files are the source of truth and are never affected by this shedding.

Progress is legible without flooding. A `phase` event marks each stage transition (`bin-discovery` → `scan` → `reconciliation` → `report`); a `unit` `action:"start"` line precedes each branch actually scanned, paired with its `scanned` result. On a resume most branches are unchanged, so by default the per-unit `skip-current`/`skip-cutoff` lines are **rolled up** into one `repo-done` event per repository (`scanned`, `reused`, `skippedCutoff`, `errored`, `deferred`) rather than printed individually — pass `--verbose-units` to emit the per-unit lines too.

During a long quiet stretch — a throttle pause, a slow clone, a large listing — a run-scoped `heartbeat` event (carrying `phase`, `unitsDone`, `elapsedSec`, `retryTotal`, `suppressed`) is emitted on a fixed cadence (`timeouts.heartbeatSeconds`, default 30) so you can tell a legitimate wait from a wedge. It only speaks up when nothing else was logged since its last tick, and it proves liveness at `await` boundaries only — Bun is single-threaded, so it cannot fire *during* one synchronous burst (a large gunzip, a scan pass, a big database write).
...
6. **Sanitized child environment (allowlist)** ([scripts/github.ts](scripts/github.ts), `buildGhEnv`/`buildGitEnv`/`buildTarEnv`): `gh`/`git`/`tar` run with a strict env allowlist, not your inherited environment. Every child gets only `HOME` and `PATH`; on top of that, `gh` and `git` also receive two classes copied from your environment — **auth** (`GH_TOKEN`/`GITHUB_TOKEN`/enterprise variants, `GH_CONFIG_DIR`/`XDG_CONFIG_HOME`) and **transport** (the `HTTP(S)_PROXY`/`NO_PROXY`/`ALL_PROXY` family in both upper- and lower-case; the OpenSSL `SSL_CERT_FILE`/`SSL_CERT_DIR` pointers; for `git` also the libcurl `GIT_SSL_CAINFO`/`GIT_SSL_CAPATH`/`CURL_CA_BUNDLE` CA-bundle pointers; and `DBUS_SESSION_BUS_ADDRESS`/`XDG_RUNTIME_DIR` for the OS keyring the credential helper reads). `tar` receives neither class — only `HOME`/`PATH`. Terminal-behavior vars (`NO_COLOR`, `TERM`, the pagers, the prompt-disable flags) are **not** copied from your environment — they are pinned to fixed non-interactive values. Everything else is dropped — including `GIT_ASKPASS`/`GIT_SSH_COMMAND`/`TAR_OPTIONS` and any other injection vector — and git additionally pins **all** of its config (system → `/dev/null`, global → a generated read-only credential-helper config). The transport vars are copied so the tool works behind a corporate proxy or a TLS-inspecting root; they govern only *how it connects*, and cannot grant write access, inject git config, or redirect where it writes (guarantees 1, 4 still hold). One assumption this rests on: the inherited `PATH` is what resolves the `gh`/`git`/`tar` binaries in the first place, so the tool trusts the local binaries on your `PATH` — a hostile `gh`/`git`/`tar` shadowing them earlier on `PATH` is outside what the argv guard can catch. Registry packument/tarball fetches use Bun's built-in `fetch`, which honors `HTTPS_PROXY` and the system trust store; `NODE_EXTRA_CA_CERTS` support is Bun-version-dependent (the ≥ 1.1 floor doesn't guarantee it), so behind a TLS-inspecting proxy prefer `SSL_CERT_FILE` or a newer Bun.
7. **See for yourself**: `bun run audit --plan` exercises auth, discovery, and scoping while writing nothing at all.
