...
`typescript` is pinned to the **6.x** line, and that is a hard constraint rather than caution. The scanners need the *in-process JS compiler API* — `createSourceFile` + `forEachChild` and the `SyntaxKind` / `ScriptKind` / `ScriptTarget` enums. TypeScript 7 is the native (Go) port: its npm root entry exports only `{ version, versionMajorMinor }`, and nothing it ships replaces that call (`typescript/unstable/ast` has the enums and node guards but no parser; `typescript/unstable/sync` is a `Program` API that proxies to the native binary over IPC). A major bump is a port, not an upgrade, so [.github/dependabot.yml](.github/dependabot.yml) ignores `typescript` majors while minor and patch updates still flow. The constraint is enforced rather than asserted. [scripts/tsCompilerApi.ts](scripts/tsCompilerApi.ts) is the package's sole import site and checks the surface at module load — before any scan, so an absent parser fails the run with a named remediation instead of degrading into a confident, empty audit. [scripts/tsCompilerApi.test.ts](scripts/tsCompilerApi.test.ts) pins the rest, and — unlike the dashboard's grep tripwires — it does so by parsing rather than by matching text. It drives the check against TypeScript 7's real root shape; proves via the AST that the check is a top-level statement sitting between the import and the export (delete that line and the module silently becomes a plain re-export); and derives the used-member list from the scanners' own syntax, so a `ts.` call a future release drops is caught in CI with the member named.

The sole-import-site scan reads every file in the repo carrying an executable extension, tests included, and collects the module specifiers each one names: static imports, re-exports, `import x = require`, type-position imports (JSDoc ones too), the dynamic-import operator, and `require` in its bare, member, and computed spellings — through parentheses and casts. Because it parses, an import written inside a comment or a string is not mistaken for one, and no formatting between tokens hides a real one. What it does **not** do is follow dataflow: a loader obtained indirectly — `createRequire`'s return value, `const load = require` — has no syntactic signature, so `createRequire` is banned outright and the rest is left to code review, the same division of labour the dashboard's scans declare for theirs. Where the scan must guess it guesses toward flagging: any `.require` call counts regardless of receiver, so a harmless `loader.require("typescript")` trips it. That is the intended direction — a false positive is noise, a false negative is a hole.
