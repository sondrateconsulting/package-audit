...
export const summarySchema = z
  .strictObject({
    ...
    branchesErrored: z.number().int().min(0).describe("DISTINCT branches whose scan FAILED this run — the UNION of two DISJOINT sets: (a) error-status run_unit_head heads (a permanent scan failure, recorded as a TERMINAL disposition at the observed commit), and (b) branches carrying a scope='scan' errors[] entry that hold NO run_unit_head row (a failure BEFORE any disposition: discovery-time, or a resume whose earlier invocation errored before a row). An error head IS a row, so it is excluded from (b) — the two never double-count. A branch with a SCANNED row PLUS a scope='scan' errors[] entry (a post-persistence log/queue write throwing after the scanned row committed) counts under branchesScanned, not here. A THROTTLE/network/service DEFERRAL is NOT a failure — it is un-covered, surfaced via runOutcome, and excluded here. On a RESUMED run, (b) still diverges from 'every branch whose scan errored' in BOTH directions: an earlier invocation's rowless error persists (errors[] is append-only, never reconciled) while rows are pruned"),
    totalDependencyFindings: z.number().int().min(0),
    totalUsageFindings: z.number().int().min(0),
  })
  .describe("Per-run disposition counts + finding totals, from the immutable run_unit_head snapshot. scanned + skippedByCutoff + excludedByPolicy + pastCap + errored account for every branch that reached a TERMINAL outcome this run (errored = error-status heads + rowless scan errors; see branchesErrored) — an EQUALITY on a single-invocation run, and an UPPER BOUND on a resumed run, where an earlier invocation's rowless error persists in errored even when the final invocation recorded no row (errors[] is append-only; rows are pruned). A DEFERRED branch (deferred-throttle, or the deferred-network/deferred-service dispositions) is NOT terminal — un-covered, finished on a later run, and surfaced via runOutcome, so it appears in NONE of these counts; that carve-out is absolute only within a single invocation, since a branch that errored in an earlier one carries an append-only error and so is counted despite being deferred in the final one");
