...
// readOnlyGuard). Pins formatVersion below so the schema is a TRUE shape discriminator: a current-shaped
...
export const summarySchema = z
  .strictObject({
    ...
    branchesDeferred: z.number().int().min(0).describe("The throttle carve-out made visible (§4): COUNT of the reported run's config work_queue rows with scope='branch' still status='pending' when the report was built — the deferred remainder, throttle-requeued in the common case (crash-recovery resets land here too), retried by a future run WHERE it still re-enumerates the unit. A QUEUE-side total, deliberately outside the terminal partition and never folded into branchesErrored: disjoint from every other count on a single-invocation run rendered at its own completion (a historical re-render can overlap them — a later run's throttle re-pends a branch this run scanned), while on a resumed run a deferred branch can also carry an earlier invocation's append-only error or retained disposition row, so summing it into the partition would double-count. One of TWO summary counts not derived from the immutable run_unit_head slice (the deferred remainder exists nowhere else in SQLite); the other is discoveryScopesDeferred, which reads the run row's sealed evidence. branchesDeferred therefore reads as the CURRENT backlog at generation time, and re-rendering an old --run-id after later runs drain the queue reports 0. Undercounts branches a throttled owner/repo discovery never enumerated (they were never enqueued; prior-run pending rows under such a repo remain counted); a pending row whose unit is never re-enumerated — branch deleted, repo dropped from the kept set (deleted, renamed, newly archived/fork-filtered, or displaced past maxReposPerOrg), or owner no longer discovered — is never dispatched again and lingers (nothing prunes work_queue), overcounting until a run re-enumerates it — scanning it ('done'), skipping it ('skipped', the past-cap settle included), or re-attempting it (a retry can throttle again and stay pending) — or --fresh drops the queue; --rescan-branch cannot clear a row no run re-enumerates (it only sets an already-pending row pending again)"),
    discoveryScopesDeferred: z.number().int().min(0).nullable().describe("The §4 DISCOVERY blind spot, made visible: how many DISTINCT discovery scopes (one per owner repo-listing, one per repository branch-listing) exhausted their bounded RETRY/PAUSE budget during the reported run (ThrottleExhausted, which has TWO distinct mechanisms: a single call running out of retries — under GitHub rate limiting or repeated HTTP 5xx — or the RUN spending its cumulative pause budget. A network/no-response failure exhausts as a permanent error instead and is NOT counted). Such a scope enumerates NOTHING, so it enqueues no work unit and records no errors row — it is invisible to branchesDeferred (a work_queue count) AND to branchesErrored, which is why this is persisted per-run rather than derived. Counts SCOPES, not branches: one throttled owner listing can hide an unbounded number of repositories and branches, so this must NEVER be summed with branchesDeferred or any branch count. Opposite currency to branchesDeferred: this is IMMUTABLE evidence sealed by completeRun once the owner pool drained (re-rendering an old --run-id cannot launder it clean), while branchesDeferred is the whole config's CURRENT pending backlog re-read at generation time. NULL means UNKNOWN, never 'none' — the run is still running, it failed, it predates schema v5 (migrated rows backfill NULL by construction), or it was RESUMED and its completing invocation saw no throttles (the accumulator is per-invocation and in memory, so that invocation cannot vouch for an earlier one); a surface that renders NULL as 0 asserts the very completeness this field exists to disprove. On a fresh run a positive value is exact; on a RESUMED run it is a LOWER BOUND — what the completing invocation counted. Either a positive value here or a positive branchesDeferred makes the run's scanned counts PARTIAL"),
    totalDependencyFindings: z.number().int().min(0),
    totalUsageFindings: z.number().int().min(0),
  })
  .describe("Per-run disposition counts + finding totals. The disposition counts come from the immutable run_unit_head snapshot; TWO fields do not — branchesDeferred reads the mutable cross-run work_queue, and discoveryScopesDeferred reads the reported run's own sealed evidence. The four disposition counts partition the RECORDED run_unit_head rows exactly once each (exact, unconditionally); together with branchesErrored they account for the branches that reached a TERMINAL outcome (scanned + skippedByCutoff + excludedByPolicy + pastCap + errored) — as an EQUALITY on a single-invocation run, and as an UPPER BOUND on a resumed run, where a branch that errored in an earlier invocation and reached no row-bearing disposition in the final one is still counted in errored (see branchesErrored, whose relationship to 'branches whose scan errored' is looser than its name suggests on a resume, in BOTH directions). A branch whose scan was THROTTLE-REQUEUED is deferred, not terminal — it holds no row and no error, and is retried by a later run that re-enumerates it, so it appears in NO PARTITION count; branchesDeferred is the queue-side count that surfaces that remainder. The carve-out is absolute only within a single invocation, since a branch that errored in an earlier one carries an error and so is counted despite being deferred");
