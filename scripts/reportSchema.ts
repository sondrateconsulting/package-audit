...
export const summarySchema = z
  .strictObject({
    ...
    branchesErrored: z.number().int().min(0).describe("DISTINCT branches whose scan ERRORED this run: they hold no run_unit_head disposition row and carry a scope='scan' errors[] entry. EXACT on a single-invocation run; an UPPER BOUND on a RESUMED one — errors[] is append-only and never reconciled, while run_unit_head rows are pruned, so a branch that errored in an earlier invocation and reached no row-bearing disposition in the final one (deleted, throttle-requeued, or its repo's discovery failed) is still counted. It never undercounts"),
    totalDependencyFindings: z.number().int().min(0),
    totalUsageFindings: z.number().int().min(0),
  })
  .describe("Per-run disposition counts + finding totals, from the immutable run_unit_head snapshot. The four disposition counts partition the RECORDED run_unit_head rows exactly once each (exact, unconditionally); together with branchesErrored they account for the branches that reached a TERMINAL outcome (scanned + skippedByCutoff + excludedByPolicy + pastCap + errored) — as an EQUALITY on a single-invocation run, and as an UPPER BOUND on a resumed run, where a branch that errored in an earlier invocation and reached no row-bearing disposition in the final one is still counted in errored (see branchesErrored). A branch whose scan was THROTTLE-REQUEUED is deferred, not terminal — it holds no row and no error, and is finished on the next run, so it appears in no count here; that carve-out is absolute only within a single invocation, since a branch that errored in an earlier one carries an error and so is counted despite being deferred");
