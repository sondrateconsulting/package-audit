// benchFrame.ts — PURE byte-level parsers for the benchmark's framed binary seam (ADR-0001
// resolution plan §3.2, prototyped as a Step-B deliverable per §4.1). No I/O and no process
// machinery here: the spawn module feeds bytes in, these parsers validate them fail-closed.
//
// Two surfaces, matching the two proposed transport reads:
...
//   • parseLsTreeZ — `ls-tree -r -z -l --full-tree` output: NUL-delimited records split at the
//     FIRST TAB (metadata left, path bytes right — a legal git path may itself contain TAB or
//     LF, which then flow into path validation, not record framing), with a closed validation
//     set over modes, types, object-format oids, sizes, and canonical unique UTF-8 paths.
//
// Both fail CLOSED on anything outside their grammar — the production spawn path's irreversible
// UTF-8 decode would destroy exactly the evidence these parsers must reject on, which is why the
// bench consumes bytes (plan §3.2).

import type { BenchObjectFormat } from "./benchGrammar.ts";
import { OID_LENGTH } from "./benchGrammar.ts";

export class BenchFrameError extends Error {
  // machine-checkable discriminant for tests and for the driver's failure taxonomy
  readonly code: string;
  constructor(code: string, message: string) {
    super(`BENCH FRAME: ${message}`);
    this.name = "BenchFrameError";
    this.code = code;
  }
}
...
// strict UTF-8: any invalid sequence throws (no replacement characters — plan §3.2 "no
// mojibake keys"); the caller converts the throw into its fail-closed error.
...
function asciiField(bytes: Uint8Array, what: string): string {
  ...
  for (const b of bytes) {
    if (b < 0x20 || b > 0x7e) throw new BenchFrameError("meta-nonascii", `${what} contains a non-printable/non-ASCII byte`);
...
export interface BatchLimits {
  // bounded pre-LF header: a header not terminated within this many bytes is fatal (plan §3.2)
  maxHeaderBytes: number;
  // absolute per-frame ceiling — production's spawn-output cap, so an ungated manifest can
  // never allocate more than today's REST path could return before the cap kill (plan §3.1)
  frameCeiling: number;
}
...
export type BatchFrame =
  ...
  // the DRIVER fails the unit closed on it (plan §3.1) — the parser only reports the frame.
...
// writing the request line (the seam is pull-style, one read in flight — plan §3.1), then feeds
// every stdout chunk through push() until it yields the frame. Memory stays O(one frame): the
// body buffer is allocated once at header acceptance, bounded by the declared size under the
// absolute ceiling.
export class BatchFrameParser {
  private readonly format: BenchObjectFormat;
  private readonly limits: BatchLimits;
  private state: BatchState = { at: "idle" };
  private pending: Uint8Array = new Uint8Array(0);

  constructor(format: BenchObjectFormat, limits: BatchLimits) {
    if (!Number.isSafeInteger(limits.maxHeaderBytes) || limits.maxHeaderBytes < 8)
      throw new BenchFrameError("limits", "maxHeaderBytes must be a safe integer >= 8");
    if (!Number.isSafeInteger(limits.frameCeiling) || limits.frameCeiling < 0)
      throw new BenchFrameError("limits", "frameCeiling must be a nonnegative safe integer");
    this.format = format;
    this.limits = limits;
  }
  ...
  private fail(code: string, message: string): never {
    ...
    throw new BenchFrameError(code, message);
  }
  ...
  // refusal happens BEFORE any request is written (plan §3.1).
  ...
  // Any grammar violation throws (and poisons the parser — the child must be killed, plan §3.2).
  push(chunk: Uint8Array): BatchFrame | null {
    ...
    for (;;) {
      if (this.state.at === "header") {
        ...
        // the per-frame bound is the ls-tree-declared size EXACTLY (plan §3.1): a header
        // disagreeing with the enumeration is a protocol violation in either direction.
...
// exact mode→type coherence over the closed set (plan §3.2)
...
// Parse the COMPLETE ls-tree -r -z -l --full-tree output (bytes, never a pre-decoded string).
// Record grammar: `<mode> SP <type> SP <oid> SP+ <size|-> TAB <pathbytes> NUL` — the -l size
// column is space-padded, hence SP+. Split at the FIRST TAB; path bytes run to the NUL and may
// legally contain TAB or LF (they then flow into path validation). Fail-closed on: unknown
// mode, mode↔type incoherence, wrong-format oid, non-canonical size, invalid-UTF-8 /
// non-canonical / duplicate paths, oversized records, entry-count overflow, and any trailing
// bytes after the last NUL.
export function parseLsTreeZ(bytes: Uint8Array, format: BenchObjectFormat, limits: LsTreeLimits): LsTreeEntry[] {
  ...
  while (offset < bytes.byteLength) {
    ...
    if (nulAt === -1)
      throw new BenchFrameError("record-unterminated", "trailing bytes after the last NUL-terminated record");
    const record = bytes.subarray(offset, nulAt);
    offset = nulAt + 1;
    if (record.byteLength > limits.maxRecordBytes)
      throw new BenchFrameError("record-bound", `record exceeds ${limits.maxRecordBytes} bytes`);
    if (entries.length >= limits.maxEntries)
      throw new BenchFrameError("entry-bound", `listing exceeds ${limits.maxEntries} entries`);
    const tabAt = record.indexOf(TAB);
    if (tabAt === -1) throw new BenchFrameError("record-shape", "record has no TAB metadata/path separator");
    ...
    if (sp1 === -1 || sp2 === -1 || sp3 === -1)
      throw new BenchFrameError("meta-shape", `metadata is not four fields: ${JSON.stringify(meta.slice(0, 120))}`);
    ...
    if (!(LS_TREE_MODES as readonly string[]).includes(modeText))
      throw new BenchFrameError("mode", `mode ${JSON.stringify(modeText)} is outside the closed set`);
    const mode = modeText as LsTreeMode;
    const type = MODE_TYPE[mode];
    if (typeText !== type)
      throw new BenchFrameError("mode-type", `mode ${mode} must carry type ${type}, got ${JSON.stringify(typeText)}`);
    if (oid.length !== OID_LENGTH[format] || !/^[0-9a-f]+$/.test(oid))
      throw new BenchFrameError("oid", `oid is not a full lowercase ${format} object id`);
    const sizeText = sizeField.replace(/^ +/, "");
    if (sizeText.includes(" "))
      throw new BenchFrameError("size-shape", `size field carries interior spaces: ${JSON.stringify(sizeField)}`);
    let size: number | null;
    if (type === "blob") {
      if (!/^(0|[1-9][0-9]*)$/.test(sizeText))
        throw new BenchFrameError("size-shape", `blob size is not a canonical nonnegative integer: ${JSON.stringify(sizeText)}`);
      size = Number(sizeText);
      if (!Number.isSafeInteger(size))
        throw new BenchFrameError("size-shape", "blob size overflows the safe-integer range");
    } else {
      if (sizeText !== "-")
        throw new BenchFrameError("size-shape", `non-blob size must be \`-\`, got ${JSON.stringify(sizeText)}`);
      size = null;
    }
    ...
    try {
      ...
      throw new BenchFrameError("path-utf8", "path bytes are not valid UTF-8 (failing closed — no mojibake keys)");
    }
    if (!isCanonicalPath(path))
      throw new BenchFrameError("path-canonical", `non-canonical path ${JSON.stringify(path.slice(0, 200))}`);
    if (seen.has(path)) throw new BenchFrameError("path-duplicate", `duplicate path ${JSON.stringify(path.slice(0, 200))}`);
    seen.add(path);
...
// ring that keeps the most recent bytes; overflow is noted, never fatal (plan §3.2).
export class ByteRing {
  ...
  constructor(cap: number) {
    if (!Number.isSafeInteger(cap) || cap < 1) throw new BenchFrameError("limits", "ring cap must be a positive safe integer");
    this.cap = cap;
  }
